...
| 数据标签 | 表数据浏览：小表（≤2 万行）一次性全量加载 + 本地翻页/排序；大表服务端分页 + 懒统计总行数。**每页行数 100/200/500/1000**、**行首勾选 + 批量删除**、双击单元格内联编辑、右键删行；（引擎定位键 `rowid` 由客户端自动携带但**不展示**）**所有改动先暂存**（改过的格子/行号标色、新增行标 +、删除行划线），左下角 `✓ 提交(n)` 单事务统一写库（任一行乐观校验失败整体回滚）、`✕ 回退` 一键丢弃 |
...
- 测试分两层：
  ...
  - **前端行为层** `web/_selftest/`（零依赖、零 npm）：在真实浏览器里 import 真实模块、派发真实事件、
    把断言写回页面。grid 61 / editor 17 / panels 18 / layout 40 / app 集成 39，共 **175 项**。
