...
namespace cella::storage
{
...
#ifdef _WIN32
  uint64_t LockNameHash(const std::string &path)
  {
  ...
  Status FileStorage::AcquireFileLock(const std::string &path)
  {
#ifdef _WIN32
    const std::string absolute = std::filesystem::absolute(path).lexically_normal().string();
    const std::wstring name = L"Local\\cella_db_" + std::to_wstring(LockNameHash(absolute));
    HANDLE handle = CreateMutexW(nullptr, TRUE, name.c_str());
    if (handle == nullptr)
    {
      return Status::Error(StatusCode::kIoError, "无法独占数据库文件: " + path);
    }
    if (GetLastError() == ERROR_ALREADY_EXISTS)
    {
      CloseHandle(handle);
      return Status::Error(StatusCode::kIoError, "数据库已被其他进程打开: " + path);
    }
    lock_handle_ = handle;
#else
    (void)path;
  ...
  void FileStorage::ReleaseFileLock()
  {
#ifdef _WIN32
  ...
  Status FileStorage::Open(const StorageConfig &config)
  {
    ...
    const Status lock_status = AcquireFileLock(path);
    if (!lock_status.ok())
    {
      return lock_status;
    }
    ...
    opened_ = true;
    const Status directory_status = load_directory(); // 4. 从磁盘读回表目录（重启后表还在）
  ...
  Status FileStorage::load_directory()
  {
    ...
    if (blob_len == 0 || blob_len > config_.page_size - 4)
    {
      return Status::Error(StatusCode::kCorruptPage,
                           "目录页长度非法: " + std::to_string(blob_len));
    }
