...
//   	xn     事务表             \clear              \timing on|off      \echo on|off
//   \help    同 \?
#include <algorithm>
#include <fstream>
#include <iostream>
...
namespace
{
  ...
  void PrintUsage(std::ostream &os)
  {
    os << "cella 数据库系统（编译器 + 存储引擎 + 数据库层整合版）\n"
          ...
          "  -h, --help          显示本帮助\n"
          "\n"
          "REPL 元命令: \\? \\q \\d [表] \\l \\plan <SQL> \\stats \\locks \\waitfor \\txn \\clear \\timing \\echo\n"
  ...
  bool ParseArgs(int argc, char **argv, Options *opt, std::string *err)
  {
    for (int i = 1; i < argc; ++i)
    {
      ...
      {
        if (!need_value(&opt->engine.data_dir))
          return false;
      }
      else if (a == "-f" || a == "--file")
      {
      ...
      {
        if (!need_value(&opt->engine.db_file))
          return false;
      }
      else if (a == "--page-size")
      {
      ...
      {
        if (!need_value(&opt->engine.replacer))
          return false;
      }
      else if (a == "--log")
      {
