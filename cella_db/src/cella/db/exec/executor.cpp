...
DbStatus FromStorage(const storage::Status& s, const std::string& what) {
  ...
  switch (s.code()) {
    case storage::StatusCode::kTableNotFound:      code = DbCode::kTableNotFound; break;
    case storage::StatusCode::kTableAlreadyExists: code = DbCode::kTableExists; break;
    case storage::StatusCode::kRecordTooLarge:     code = DbCode::kRecordTooLarge; break;
    case storage::StatusCode::kTypeMismatch:       code = DbCode::kTypeMismatch; break;
    case storage::StatusCode::kInvalidArgument:    code = DbCode::kStorageError; break;
    default:                                       code = DbCode::kStorageError; break;
  }
...
DbStatus Executor::ExecCreateTable(const cella::CELLA_PlanNode& plan, const ExecContext& ctx,
  ...
  cella::CELLA_Table for_schema;
  for_schema.name = st->tableName;
  for (const auto& cd : st->columns) {
    ...
    cc.type = cd.type;
    cc.len = c.len;
    cc.notNull = cd.notNull;
    for_schema.columns.push_back(std::move(cc));
  }

  const storage::Schema schema = ToStorageSchema(for_schema);
  {
