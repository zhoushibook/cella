...
namespace cella::db
{
  namespace
  {
    ...
    double AsDouble(const Value &v)
    {
      ...
      case ValueType::kInt32:
        return static_cast<double>(v.int32_val);
      case ValueType::kInt64:
        return static_cast<double>(v.int64_val);
      case ValueType::kFloat:
        return static_cast<double>(v.float_val);
  ...
  ValueType ToStorageType(cella::CELLA_DataType t)
  {
    ...
    case cella::CELLA_DataType::TEXT:
      return ValueType::kVarchar;
    case cella::CELLA_DataType::DATE:
      return ValueType::kVarchar;
    case cella::CELLA_DataType::TIME:
      return ValueType::kVarchar;
  ...
  Schema ToStorageSchema(const CatalogTable &table)
  {
    ...
    for (const auto &c : table.columns)
    {
      schema.AddColumn(c.name, ToStorageType(c.type),
                       StorageMaxLen(c.type, c.len, c.len > 0));
    }
    return schema;
  }
