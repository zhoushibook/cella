...
namespace cella::db::wal
{
  ...
  bool DecodePayload(RecordType type, const char *data, size_t size, WalRecord *out)
  {
    ...
    if (!rd.TakeU8(&t))
      return false;
    if (static_cast<RecordType>(t) != type)
      return false;
    out->type = type;
    switch (type)
    {
    ...
    case RecordType::kUpdate:
    {
      if (!rd.TakeU64(&out->txn_id))
        return false;
      if (!rd.TakeStr(&out->table))
        return false;
      if (!rd.TakeU32(&out->page_id))
        return false;
                            ...
                            size_t *truncated) const
  {
    ...
    while (pos + kRecordHeaderSize <= all.size())
    {
      ...
      lsn_t lsn = 0;
      if (!rd.TakeU32(&magic) || !rd.TakeU32(&size) || !rd.TakeU32(&crc))
        break;
      if (!rd.TakeU64(&lsn))
        break;
      if (magic != kRecordMagic)
        break;
      if (size < kRecordHeaderSize || pos + size > all.size())
        break;
      ...
      if (!prd.TakeU8(&t))
        break;
      WalRecord rec;
      if (!DecodePayload(static_cast<RecordType>(t), payload, payload_len, &rec))
        break;
      rec.lsn = lsn;
      if (out != nullptr)
  ...
  DbStatus WalManager::FlushDurable()
  {
    std::lock_guard<std::recursive_mutex> lk(mu_);
    if (!opened_)
    {
      return DbStatus::Error(DbCode::kWalError, "WAL 未打开，无法同步刷盘");
    }
    if (!buffer_.empty())
    {
      out_.write(buffer_.data(), static_cast<std::streamsize>(buffer_.size()));
      ...
      if (!out_)
      {
        return DbStatus::Error(DbCode::kWalError, "WAL 写入失败: " + path_);
      }
      buffer_.clear();
      ++flush_count_;
  ...
  DbStatus WalManager::SyncFile()
  {
#if defined(_WIN32)
    HANDLE file = CreateFileA(path_.c_str(), GENERIC_READ | GENERIC_WRITE,
                              FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE,
                              nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, nullptr);
    if (file == INVALID_HANDLE_VALUE)
    {
      return DbStatus::Error(DbCode::kWalError, "WAL 文件打开失败，无法同步刷盘: " + path_);
    }
    const BOOL ok = FlushFileBuffers(file);
    CloseHandle(file);
    if (!ok)
    {
      return DbStatus::Error(DbCode::kWalError, "WAL FlushFileBuffers 失败: " + path_);
    }
#else
    ...
    if (result != 0)
    {
      return DbStatus::Error(DbCode::kWalError, "WAL fsync 失败: " + path_);
    }
#endif
    return DbStatus::Ok();
  }
