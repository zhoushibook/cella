...
1. [x] `IsDatabaseControl()`（**前两词匹配**，别误吞 `CREATE TABLE`）+ 四条语句的 Session 层解析 + 库名合法性校验（标识符规则、保留字拒绝）
2. [x] `CreateDatabase`：`<data_dir>/<name>.db` 不存在 → 初始化（复用 `Open()` 的存储引导路径）；已存在 → `DB-514`
3. [x] `DropDatabase`：库不存在 → `DB-514`；目标是当前库或 main → `DB-515`；否则把 `<db>.db` 改名 `<db>.db.dropped-<时间戳>`（软删除，可手动恢复）
4. [x] `ShowDatabases`：列 `<data_dir>/*.db`（`main` 恒在），返回 `QueryResult`（单列 name）
5. [x] `UseDatabase`：库存在性检查 → 事务中拦截（DB-513）→ `Checkpoint()` 当前库 → 换 `db_file` 重开 storage → `LoadFromStorage()` → 更新会话当前库名
6. [x] 锁与 undo 的资源名加库前缀（全量过一遍 `LockTable` 调用点 + undo 记录）
7. [x] CLI：提示符/`\l`/帮助
8. [x] 旧布局迁移（根下散落 `cella.db` → `main.db`，提示一次）
9. [ ] 测试与文档

### 3.3b 阶段二完成记录（2026-09-11）

* 全部步骤完成；回归：编译器 29/29、存储层 30/30（4620 断言）、整合层 **83/83（697 断言）** 全绿。
* 与计划的两处偏差：
  1. 「当前库/启动库都不可删」取代「main 不可删」——默认文件名已从 cella.db 改为 main.db，
     软删除本身可恢复，故只拦当前库与启动库（DB-515）。
  2. 实施中发现的关键坑：**USE 之后必须 `EnsureCatalogInSync()` 重建编译器目录**，
     否则后续语句仍按旧库的表结构做语义分析（静默错乱、语句莫名失败）——
     这是拦截式多库最容易踩的坑，已写死在会话层代码注释里。
* 编译器、存储层零改动（与承诺一致）；undo 记录保持裸表名（回滚只发生在当前库内），
  仅锁资源名加库前缀，比计划更收敛。
...
1. [x] 全量 `run_all.ps1` 回归（编译器 29 / 存储层 30 / 整合层全部用例）
2. [x] 指导书符合性核对：三条缺口逐条对照原文关闭（目录表 / 页式持久化 / 多库）
3. [x] 文档四处同步（INTEGRATION / ARCHITECTURE / README / TEST_REPORT），并在 INTEGRATION §5 记录本次改动清单
4. [x] git 提交拆分：阶段一一个 commit，阶段二一个 commit（便于回退）
