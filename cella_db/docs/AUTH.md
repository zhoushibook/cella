...
* **首次创建即完整落盘**（建表与 root 引导完成后立刻刷全），进程中途被打断也不会留下
  「只有元数据页」的半成品；此后每次改动随 `Close`/存盘点落盘，最坏只丢最后一次改动。
* **每次用户/授权改动都立即刷盘**（`CreateUser` / `DropUser` / `SetPassword` / `GRANT` /
  `REVOKE` / `GRANT ADMIN` 写后即 `Close + Open` 身份库）—— 进程被强杀（Ctrl+C、关窗口、
  杀 web 服务）也不会丢：身份库改动极少，这个代价可以忽略。
* **口令不能包含换行 / 制表等控制字符**：REPL 跨行输入会把换行原样存进引号串里
  （口令「看起来对、实际多个换行」的真实坑），含控制字符的口令在入口直接拒绝。
* 若身份库文件**已损坏**（例如曾被手工截断），启动会明确报错并给出文件路径：
  把该文件改名或删除后重启即可重建（恢复默认管理员 root、口令为空），**原有用户与授权会丢失** ——
  因此请先备份再操作。
