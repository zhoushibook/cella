...
## 11. 实施完成记录（2026-09-13）

三个阶段全部按计划落地，分三次提交（M1 认证 `304bc96`、M2 授权 `3f12388`、M3 客户端见紧随其后的提交）。

回归：编译器 33/33、存储 30/4620（4620 断言）、整合 **105/105（970 断言）**、
客户端 **29/29（201 断言）**、run_all 的 access control smoke 7 条语句、3 个 demo、两个示例 —— 全绿。

与计划有偏差 / 实施中暴露的问题（均已处理）：

1. **DDL 权限检查的提前返回必须复位编译器目录**（`EnsureCatalogInSync()`）。
   语义阶段会把「待建表名」写进编译器目录副本，若提前返回不复位，失败的建表会让下一次同名
   建表误报「表已存在（SEM-302）」—— 这是个真 bug，已修并加了用例。
2. **`GRANT ADMIN` 不写 `ON`**：解析器最初强制要求 `ON`，与 AUTH/README 的写法不一致；
   已放行（仅当权限列表全是 `ADMIN` 时允许省略，其它权限仍必须有 `ON`）。
3. **判定语义以「并集」实现**（任一覆盖该需求的授权即通过，与 MySQL 一致、没有「拒绝」），
   而非计划里「取最具体一条」的字面表述 —— 后者易被误读为存在 deny；AUTH.md 已改写说明。
4. **客户端部分端点原本直连引擎**（建库/删库/存盘点/服务端诊断/目录），会**绕过权限**。
   为此把 `Session::RequireAdmin` / `CheckDatabaseAccess` / `FilterDatabasesByPrivilege`
   提升为公开入口，由这些端点复用同一套判定（越权 → HTTP 403 + DB-802；库列表按权限过滤）。
5. **锁的约定**：`AuthStore` 内部互斥量是**叶子锁**（只保护内存缓存），触达存储的写操作要求
   调用方已持 `DbEngine::storage_mutex_`；锁序固定 `storage_mutex_` → `AuthStore::mutex_`。
   客户端 `gate_` 改为**递归**互斥量，以便「校验令牌 + 注入身份 + 调用处理器」在同一临界区内完成。


---

## 12. 事后修复：半成品身份库（2026-09-13 17:3x，用户现场）

**现象**：在已有数据目录上第一次开 `--auth`，报 `DB-520 身份库打开失败: kPageNotFound: 读取页失败: 2`，
之后每次都失败。现场文件 `cella_auth.db` 只有 **4096 字节（1 页，仅元数据页）**，而元数据里已引用页 2；
健康身份库应为 **16384 字节（4 页）**。日志里没有任何 `[auth]` 记录 → 进程在「打开身份库」与
「建表落盘」之间被打断（元数据页先写盘、表页只随 Close 刷出）。

**修复**（两处）：
1. `EnsureAuthOpen` 在首次创建（`created == true`）并完成 root 引导后，立即 `Close + Open` 强制刷全
   （与存盘点同一手法），把「半成品」窗口压到接近零；此后文件始终至少自洽。
2. 打开失败的报错改为**可自救**：带上文件路径，并说明「改名/删除后重启即可重建，
   恢复默认管理员 root（口令为空），原有用户与授权会丢失」。

**回归**：新增 `认证_首次创建即完整落盘`（断言引擎开着时文件就已完整、flush 后数据可用、重开仍在）；
整合层 105→**106** 用例 / 970→**979** 断言，全量绿。现场已恢复：坏文件留档为
`mydb/cella_auth.db.broken-20260913`，重建后 `student` 数据原样。
...
## 13. 事后修复 2：口令改动即落盘 + 主库首建落盘（2026-09-13 晚，用户现场）

**现象**：CLI 里 `SET PASSWORD = '123456';` 报成功，退出重进新口令无效、旧口令依旧。
根因有两层：

1. **口令改动只进缓冲池**：身份库写操作（建/删用户、改口令、授权）都只写页式存储的脏页，
   `Close`/存盘点才落盘。退出方式不是 `\q`（Ctrl+C / 关窗口 / 强杀 web）时改动直接丢失。
   之前 17:51 那次「更新口令」就是这样丢的，造成一连串「哈希对不上」的假象。
2. **主库文件首次创建同样只有元数据页落盘**：web 建目录后被强杀，`main.db` 只剩
   4096 字节，此后每次打开都报 `kPageNotFound: 读取页失败: 2` —— 与 §12 的身份库半成品
   同族，只是发生在主存储上（复现：web 建新目录 → 建表 → 强杀 → CLI 打开失败）。

**修复**：

1. `AuthStore` 增加写后刷盘钩子（`SetFlushHook`，由 `DbEngine` 注入 `FlushAuth` =
   身份库 `Close + Open` 强制刷全）；六个写操作成功后立即触发 —— 改完即持久，强杀不丢。
2. `DbEngine::Open` 增加 ⑩：数据文件**首次创建**（打开前不存在）时，在目录引导完成后
   `Close + Open` 主存储刷全 —— 数据目录不再产生「永久打不开」的半成品。

**顺手加固**：口令含换行/制表等控制字符（< 0x20）在 `AuthStore::CheckPassword` 直接拒绝
（DB-501 带提示），堵住「跨行输入把换行存进口令」的坑。

**验证**：新增 `认证_口令改动即落盘`（引擎开着时文件里就有新哈希 + 控制字符被拒 +
重开生效）与 `多库_数据文件首次创建即完整落盘`（引擎开着时 main.db 已是完整多页）；
端到端：web 建目录 → 建表插数改口令 → `taskkill /F` 强杀 → CLI 用新口令登录成功、
表结构完好（DML 行数据丢失仍是既有设计：DML 靠 Close/存盘点，可选
`--checkpoint-on-commit`）。回归：编译器 33、存储 30/4620、整合 **108/108（989 断言）**、
客户端 29、access control smoke 7 —— 全绿。
