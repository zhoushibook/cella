...
| 8d | 主键 = 列级或表级复合 | 列级与表级主键（含复合 `PRIMARY KEY (a, b[, …])`，与列级互斥 `SEM-313`）统一自动建唯一索引 `<表名>_pk`（B+ 树，复合键按声明序编码）；唯一性由索引元组前缀比较保证（扫描查重保留为兜底）；**主键列序 = 声明序**（决定索引键序与最左前缀语义） |
| 8d' | 二级索引 = 单列或复合 B+ 树 | `CREATE [UNIQUE] INDEX … ON t (col[, col…])` 支持多列；列清单在 `cella_index.column` 里逗号拼接落库（行格式与 golden 不变）；复合键编码 = NULL 位图 + 各列保序编码依次拼接（位图消除 NULL 标记与定宽值编码的前缀歧义）；键长上限在建索引时校验（超页拒绝）；最左前缀规则：执行器第一版只对「全键等值」下推（`ScanPrefix` 字节前缀扫描），缺列退回全表扫描；唯一索引 NULL 语义 = 标准 SQL（元组任一列为 NULL 不参与唯一判定，PK 列隐含 NOT NULL 不受影响） |
