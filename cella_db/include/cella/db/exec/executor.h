...
namespace cella::db
{
  ...
  public:
    ...
    // ── 恢复期行哈希索引（性能修复：重放定位 O(表行数) → O(1)）────
    // 背景：重放每条记录都要回答「这行是否已生效」。修复前是**逐条全表扫描**，
    // 遇到「5.7 万条 INSERT 的日志 × 5.7 万行的表」要跑数十亿次行比对 ——
    // 实测永远收敛不了，cella_web 卡在开库阶段、端口从未监听。
    // 现在：恢复期间按表建一次「键 → 行踪」哈希（O(N)，懒构建），之后定位 O(1)；
    // 重放过程中的插/删/改同步维护它。键的规则与定位规则互为镜像：
    // 有主键按主键列，无主键按整行内容（与 wal::ValueEqual 的等价关系对齐）。
    // 由 RecoveryManager 在整个恢复期间打开；关闭即释放，运行期行为不受影响。
    void BeginRecoveryMode();
  ...
  private:
    ...
    // ── 恢复期行哈希索引的实现细节（见 BeginRecoveryMode 注释）──────
    // 一张表的索引：键 → 行踪列表（无主键表允许全同重复行，所以一个键挂多行）。
    struct RecoveryRowIndex
    {
      bool by_pk = false; // true = 键取主键列；false = 键取整行内容
      int pk_idx = -1;    // 主键列下标（by_pk 时有效）
      std::unordered_map<std::string, std::vector<storage::Rid>> rows;
    };

    // 该表的行索引不存在时构建一次（全表扫描，O(N)）；只在恢复期调用。
    const RecoveryRowIndex *EnsureRecoveryRowIndex(const CatalogTable &table,
                                                   storage::TableHeap *heap);
    // 构造定位键（类型标签 + 载荷）。false = 值不足以构造键 → 退回线性扫描。
    static bool BuildRecoveryKey(bool by_pk, int pk_idx,
                                 const std::vector<storage::Value> &values, std::string *out);
    // 重放期间的行变更同步维护行索引；没有索引的表（没跑过定位）什么都不做。
    void RecoveryRowIndexAdd(const std::string &table_name,
                             const std::vector<storage::Value> &values,
                             const storage::Rid &rid);
    void RecoveryRowIndexRemove(const std::string &table_name,
                                const std::vector<storage::Value> &values,
                                const storage::Rid &rid);
    ...
    // 恢复期行哈希索引（表名 → 索引，见 BeginRecoveryMode 的注释）。
    // BeginRecoveryMode 清空、EndRecoveryMode 释放：只在崩溃恢复期间存在，
    // 运行期的任何路径都不读它（保证与修复前的行为逐字节一致）。
    bool recovery_mode_ = false;
    std::unordered_map<std::string, RecoveryRowIndex> recovery_row_index_;
